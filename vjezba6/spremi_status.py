...
# Funkcija za dohvaćanje korisničkog ID-a iz baze na temelju session_id-a
def get_user_id(session_id, cursor):
...
user_id = get_user_id(session_id, cursor)
if not user_id:
    print("Location: login.py\n")
    print()
    exit()
...
if not form.list:
    ...
    for field in form.list:
        if field.name.startswith("status_"):
            ...
            # Provjera postoji li već zapis za tog korisnika i predmet
            cursor.execute("SELECT id FROM upisni_list WHERE id_studenta = %s AND id_predmeta = %s", (user_id, subject_id))
            existing = cursor.fetchone()

            if existing:
                # Ažuriranje postojećeg zapisa
                cursor.execute("UPDATE upisni_list SET status = %s WHERE id = %s", (status, existing[0]))
            else:
                # Umetanje novog zapisa
                cursor.execute("INSERT INTO upisni_list (id_studenta, id_predmeta, status) VALUES (%s, %s, %s)",
                               (user_id, subject_id, status))

    db.commit()

    # Nakon spremanja, prikaz poruke
    print("""
    <!DOCTYPE html>
    <html lang="hr">
    <head>
